...
Supongamos que nos interesa: \[I = \int_0^1h\left( x\right) dx\] Si
\(x_1,x_2,\ldots ,x_n\) \emph{i.i.d.} \(\mathcal{U}\left( 0,1\right)\)
entonces:
...
y la aproximación por simulación sería \texttt{sum(w\ *\ h(y))} (media
ponderada) y en el análisis de convergencia se emplearía
\texttt{cumsum(w\ *\ h(y))} (sin dividir por el número de simulaciones).
