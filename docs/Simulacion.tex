...
simulación}\label{ventajas-e-inconvenientes-de-la-simulacion}
...
\chapter{Generación de números pseudoaleatorios con distribución
uniforme}\label{cap3}
...
Se basan en la idea de considerar una combinación lineal de los últimos
\(k\) enteros generados y calcular su resto al dividir por un entero
fijo \(m\). En el método congruencial simple (de orden \(k = 1\)),
partiendo de una semilla inicial \(x_0\), el algoritmo secuencial es el
...
\BeginKnitrBlock{theorem}
\protect\hypertarget{thm:unnamed-chunk-5}{}{\label{thm:unnamed-chunk-5} } Un
...
\ref{fig:randu}). Por ejemplo, el conjunto de datos \texttt{randu}
contiene 400 tripletas de números sucesivos obtenidos con la
implementación en VAX/VMS 1.5 (1977).
...
\item
  ...
  \(u_{i}=\sum_{i=1}^L u_{i}^{(l)} \bmod 1\), donde \(u_{i}^{(l)}\) es
...
Kolmogorov-Smirnov, implementado en \texttt{ks.test}. En la Sección
\ref{gof} se describen con más detalle estos contrastes.
...
\begin{center}\includegraphics[width=0.7\linewidth]{03-Generacion_numeros_aleatorios_files/figure-latex/unnamed-chunk-20-1} \end{center}
...
\end{Shaded}
  ...
  \begin{center}\includegraphics[width=0.7\linewidth]{05-Metodos_generales_continuas_files/figure-latex/unnamed-chunk-14-1} \end{center}
\item
  Repetir el apartado anterior con \(n=100\).
\end{enumerate}

\section{Modificaciones del método de aceptación
rechazo}\label{modificaciones-del-metodo-de-aceptacion-rechazo}
...
llamada también distribución Gamma--Poisson, o la distribución
...
distribuciones
notables}\label{metodos-especificos-para-la-generacion-de-algunas-distribuciones-notables}
...
\iffalse (de transformación cuantil) \fi{} } \vspace{0.5cm}
...
\section{Método de la tabla guía}\label{metodo-de-la-tabla-guia}
...
\protect\hypertarget{cnj:unnamed-chunk-20}{}{\label{cnj:unnamed-chunk-20}
\iffalse (de simulación mediante una tabla guía) \fi{} } \vspace{0.5cm}
...
anterior empleando esta rutina con \(m=n-1\).
...
cuantil}\label{calculo-directo-de-la-funcion-cuantil}
...
\protect\hypertarget{exm:unnamed-chunk-2}{}{\label{exm:unnamed-chunk-2}
\iffalse (simulación de normales independientes) \fi{} }
\EndKnitrBlock{example}
...
\protect\hypertarget{cnj:unnamed-chunk-3}{}{\label{cnj:unnamed-chunk-3}
...
\end{enumerate}
\EndKnitrBlock{conjecture} Las funciones implementadas en el paquete
base de \texttt{R} permiten simular fácilmente en el caso independiente.
Por ejemplo en el caso bidimensional con
...
\KeywordTok{curve}\NormalTok{(f1, }\OperatorTok{-}\DecValTok{3}\NormalTok{, }\DecValTok{3}\NormalTok{, }\DataTypeTok{ylim =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\KeywordTok{f2}\NormalTok{(}\OperatorTok{-}\DecValTok{1}\NormalTok{)), }\DataTypeTok{ylab =} \StringTok{"f_i(x)"}\NormalTok{)}
...
\caption{Densidades marginales de las componentes.}\label{fig:unnamed-chunk-4}
\end{figure}
...
El algoritmo de aceptación-rechazo sería exactamente el mismo, lo único
que cambiaría serían que las densidades son multidimensionales.
Supongamos que la densidad objetivo \(f\) y la densidad auxiliar \(g\)
verifican:
...
\[Cov(A\mathbf{X}) = A\Sigma A^t.\] La idea de este tipo de métodos es
simular datos independientes y transformarlos linealmente de modo que el
resultado tenga la covarianza deseada.
...
\protect\hypertarget{cnj:unnamed-chunk-9}{}{\label{cnj:unnamed-chunk-9}
...
\item
  Hacer \(\mathbf{X} =\mathbf{\mu+}L\mathbf{Z}\).
\item
...
incondicional}\label{simulacion-condicional-e-incondicional}
...
\protect\hypertarget{cnj:unnamed-chunk-19}{}{\label{cnj:unnamed-chunk-19}
\iffalse (de simulación condicional) \fi{} } \vspace{0.5cm}
...
\caption{Realizaciones condicionales del proceso funcional.}\label{fig:unnamed-chunk-23}
\end{figure}
...
ajustado}\label{simulacion-condicional-a-partir-de-un-modelo-ajustado}
...
\subsection{Cópulas Arquimedianas}\label{copulas-arquimedianas}

Además de las cópulas Gausianas, es una de las familias de cópulas más
utilizadas. Son de la forma: \[C(x_1,x_2,\dots,x_{n})
...
fácilmente mediante el \textbf{método secuencial de distribuciones
condicionadas}.

\BeginKnitrBlock{conjecture}[de simulación bidimensional mediante cópulas]
\protect\hypertarget{cnj:unnamed-chunk-29}{}{\label{cnj:unnamed-chunk-29}
...
\ControlFlowTok{for}\NormalTok{ (j }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\NormalTok{nstarts)\{}
  ...
  \CommentTok{# For "SANN" maxit gives the total number of function evaluations: there is no other stopping criterion. Defaults to 10000.}
...
\subsection*{Artículos}\label{articulos}
...
\subsection{Gráficos P-P y Q-Q}\label{graficos-p-p-y-q-q}
...
\subsection{Contraste de
Kolmogorov-Smirnov}\label{contraste-de-kolmogorov-smirnov}
...
dependencia}\label{metodos-para-detectar-dependencia}
...
\subsection{Gráfico secuencial}\label{grafico-secuencial}
...
retardado}\label{grafico-de-dispersion-retardado}
...
aleatorios}\label{contrastes-especificos-para-generadores-aleatorios}
...
se trató en el Capítulo 12 empleando Integración Montecarlo y MCMC), nos
