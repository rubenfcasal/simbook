...
aproximadas/asintóticas) y a menudo la resolución también es rápida.
Cuando la solución no se puede obtener de modo analítico (o si la
aproximación disponible no es adecuada) se puede recurrir a la
simulación.

Podríamos decir que la \emph{Simulación} es una técnica que consiste en
realizar experimentos de muestreo sobre el modelo de un sistema, con el
objetivo de recopilar información bajo determinadas condiciones. Nos
centraremos en el caso de la \emph{Simulación Estocástica}: las
...
\href{https://cran.r-project.org/web/packages/randtoolbox/index.html}{\texttt{randtoolbox}}
...
Se obtienen mediante un algoritmo recursivo denominado \emph{generador}:
...
\href{https://cran.r-project.org/web/packages/distr/index.html}{\texttt{distr}},
...
se guardan y se procesan vectorialmente (normalmente empleando la
función \texttt{apply}). En problemas mas complejos, en los que no es
necesario almacenar todas las simulaciones, puede ser preferible emplear
un bucle para generar y procesar las muestras iterativamente. Por
ejemplo podríamos emplear el siguiente esquema:
...
entorno de trabajo\ldots{} \#\# Opciones
...
\section{Generadores congruenciales}\label{cap3-gen-cong}
...
\begin{itemize}
\item
  Si \(c=0\) se denomina congruencial \textbf{multiplicativo}
\item
  Si \(c\neq0\) se denomina congruencial \textbf{mixto}
\end{itemize}

Implementado en el fichero \emph{RANDC.R} (tratando de imitar el
funcionamiento en R, aunque de forma no muy eficiente\ldots{}):
...
recomienda emplear un algoritmo como el descrito en L'Ecuyer (1988).
\EndKnitrBlock{remark}
...
\subsection{Periodo}\label{periodo}
...
\item
  \(c\) y \(m\) son primos relativos (i.e.
  \(m.c.d.\left( c,m\right) =1\)).
\item
...
\item
  ...
  (i.e.
  \(m\equiv0\operatorname{mod}4\Rightarrow a\equiv 1\operatorname{mod}4\)).
\end{enumerate}
...
deben aparentar muestras i.i.d. \(\mathcal{U}(0,1)\). Por ejemplo, los
valores generados pueden mostrar una estructura reticular.

\begin{itemize}
\item
  Marsaglia (1968): \(k\)-uplas de generadores multiplicativos
  contenidas en a lo sumo \(\left(k!m\right)^{1/k}\) hiperplanos
  paralelos.
\item
...
\section{Factorización de la matriz de
covarianzas}\label{factorizacion-de-la-matriz-de-covarianzas}
...
\subsection{Simulación de la normal
multivariante}\label{simulacion-de-la-normal-multivariante}
...
\chapter{Integración y Optimización Montecarlo}\label{cap9}
