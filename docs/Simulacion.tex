...
\nocite{hull1962, vonneuman1951, marsaglia1977}
\IfFileExists{bookmark.sty}{\usepackage{bookmark}}{\usepackage{hyperref}}
\IfFileExists{xurl.sty}{\usepackage{xurl}}{} % add URL line breaks if available
\urlstyle{same} % disable monospaced font for URLs
\hypersetup{
  pdftitle={Técnicas de Simulación y Remuestreo},
  pdfauthor={Rubén Fernández Casal (ruben.fcasal@udc.es); Ricardo Cao (rcao@udc.es); Julián Costa (julian.costa@udc.es)},
  pdflang={es},
  hidelinks,
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Establecer semilla y parámetros}
...
\NormalTok{  tmp }\OtherTok{\textless{}{-}} \FunctionTok{freq.test}\NormalTok{(u)}
...
Una justificación teórica de la validez de esta aproximación es \emph{la ley débil de los grandes números}\footnote{\emph{La ley fuerte} establece la convergencia casi segura.}:
...
\FunctionTok{lines}\NormalTok{(est }\SpecialCharTok{{-}} \DecValTok{2}\SpecialCharTok{*}\NormalTok{esterr, }\AttributeTok{lty=}\DecValTok{3}\NormalTok{)}
\FunctionTok{abline}\NormalTok{(}\AttributeTok{h =}\NormalTok{ xmed, }\AttributeTok{col =} \StringTok{"blue"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{  rx[i] }\OtherTok{\textless{}{-}} \ControlFlowTok{if}\NormalTok{ (rx[i}\DecValTok{{-}1}\NormalTok{]) }\FunctionTok{runif}\NormalTok{(}\DecValTok{1}\NormalTok{) }\SpecialCharTok{\textgreater{}}\NormalTok{ beta }\ControlFlowTok{else} \FunctionTok{runif}\NormalTok{(}\DecValTok{1}\NormalTok{) }\SpecialCharTok{\textless{}}\NormalTok{ alpha}
  \CommentTok{\# rx[i] \textless{}{-} if (rx[i{-}1]) runif(1) \textless{} 1 {-} beta else runif(1) \textless{} alpha}
\end{Highlighting}
\end{Shaded}
...
\FunctionTok{lines}\NormalTok{(est }\SpecialCharTok{{-}} \DecValTok{2}\SpecialCharTok{*}\NormalTok{esterr, }\AttributeTok{lty=}\DecValTok{2}\NormalTok{)}
\CommentTok{\# res \textless{}{-} simres::conv.plot(rx, ylim=c(0,0.6)); res}
\FunctionTok{abline}\NormalTok{(}\AttributeTok{h =} \DecValTok{1}\SpecialCharTok{/}\DecValTok{3}\NormalTok{, }\AttributeTok{col =} \StringTok{"blue"}\NormalTok{) }\CommentTok{\# Probabilidad teórica}
\end{Highlighting}
\end{Shaded}
...
\FunctionTok{lines}\NormalTok{(est }\SpecialCharTok{{-}} \DecValTok{2}\SpecialCharTok{*}\NormalTok{esterr, }\AttributeTok{lty=}\DecValTok{2}\NormalTok{)}
\FunctionTok{abline}\NormalTok{(}\AttributeTok{h =} \DecValTok{1}\SpecialCharTok{/}\DecValTok{3}\NormalTok{, }\AttributeTok{col =} \StringTok{"blue"}\NormalTok{)     }\CommentTok{\# Prob. teor. cadenas Markov}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{mean}\NormalTok{(rx) }\CommentTok{\# Aproximación media teórica}
\end{Highlighting}
\end{Shaded}
...
\FunctionTok{lines}\NormalTok{(est }\SpecialCharTok{{-}} \DecValTok{2}\SpecialCharTok{*}\NormalTok{esterr, }\AttributeTok{lty =} \DecValTok{2}\NormalTok{)}
\FunctionTok{abline}\NormalTok{(}\AttributeTok{h =} \DecValTok{1}\SpecialCharTok{/}\DecValTok{3}\NormalTok{, }\AttributeTok{col =} \StringTok{"blue"}\NormalTok{)  }\CommentTok{\# Prob. teor. cadenas Markov}
\NormalTok{esterr[}\FunctionTok{length}\NormalTok{(rxm)]}
\end{Highlighting}
\end{Shaded}
...
\FunctionTok{abline}\NormalTok{(}\AttributeTok{h =} \DecValTok{1}\SpecialCharTok{/}\DecValTok{3}\NormalTok{, }\AttributeTok{col =} \StringTok{"blue"}\NormalTok{)     }
...
\NormalTok{x[}\DecValTok{1}\NormalTok{] }\OtherTok{\textless{}{-}} \SpecialCharTok{{-}}\DecValTok{10}   \CommentTok{\# Mejor x[1] \textless{}{-} rnorm(1, mean = xmed, sd = sqrt(xvar))}
...
Para simular una serie de tiempo (de media cero) en R se puede emplear la función \texttt{arima.sim} del paquete base \texttt{stats}.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{rx2 }\OtherTok{\textless{}{-}}\NormalTok{ xmed }\SpecialCharTok{+} \FunctionTok{arima.sim}\NormalTok{(}\FunctionTok{list}\NormalTok{(}\AttributeTok{order =} \FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{,}\DecValTok{0}\NormalTok{,}\DecValTok{0}\NormalTok{), }\AttributeTok{ar =}\NormalTok{ rho), }\AttributeTok{n =}\NormalTok{ nsim, }
...
Este número \(N\) es aleatorio y sigue una distribución geométrica (entendida como el número de pruebas necesarias hasta obtener el primer éxito\footnote{Esta distribución también se puede definir como el número de fracasos hasta el primer éxito al realizar experimentos de Bernoulli independientes, que es como la consideraremos normalmente (ya que coincide con la parametrización de R, como un caso particular de la distribución binomial negativa).
  Sin embargo, en este caso resulta más cómodo considerar el número total de pruebas.}) con parámetro \(p\) (probabilidad de éxito) la probabilidad de aceptación en el paso 3:
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{c.cuasi }\OtherTok{\textless{}{-}}\NormalTok{ emv}\SpecialCharTok{$}\NormalTok{objective}
\end{Highlighting}
\end{Shaded}
...
\caption{Ilustración del algoritmo de aceptación-rechazo con envolvente inferior.}\label{fig:squeeze}
\end{figure}
...
\NormalTok{res}\SpecialCharTok{$}\NormalTok{pteor }\OtherTok{\textless{}{-}}\NormalTok{ pmf}
\FunctionTok{print}\NormalTok{(res, }\AttributeTok{digits =} \DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Máximo error absoluto }
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Máximo error absoluto porcentual }
\DecValTok{100}\SpecialCharTok{*}\FunctionTok{max}\NormalTok{(}\FunctionTok{abs}\NormalTok{(res}\SpecialCharTok{$}\NormalTok{psim }\SpecialCharTok{{-}}\NormalTok{ res}\SpecialCharTok{$}\NormalTok{pteor) }\SpecialCharTok{/}\NormalTok{ res}\SpecialCharTok{$}\NormalTok{pteor)}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{AR-multi}{%
\section{El método de aceptación-rechazo}\label{AR-multi}}
...
\CommentTok{\# La versión actual de geoR::image.kriging() no admite múltiples gráficos con mfrow}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{sim.stat }\OtherTok{\textless{}{-}} \FunctionTok{apply}\NormalTok{(rtablas, }\DecValTok{2}\NormalTok{, }
                  \ControlFlowTok{function}\NormalTok{(x) }\FunctionTok{chisq.test}\NormalTok{(}\FunctionTok{matrix}\NormalTok{(x, }\AttributeTok{nrow =} \FunctionTok{nrow}\NormalTok{(tabla)))}\SpecialCharTok{$}\NormalTok{statistic)}
...
\[I = \int_0^1 s(x) dx\]
Por tanto, si \(x_1,x_2,\ldots ,x_n\) \emph{i.i.d.} \(\mathcal{U}(0, 1)\), entonces:
...
\protect\hypertarget{exm:mc-integral-clas}{}\label{exm:mc-integral-clas}
...
\begin{Highlighting}[]
\CommentTok{\# ·········································································· }
\CommentTok{\# Integración Monte Carlo de \textasciigrave{}fun()\textasciigrave{} entre \textasciigrave{}a\textasciigrave{} y \textasciigrave{}b\textasciigrave{} utilizando una muestra }
\CommentTok{\# (pseudo) aleatoria de tamaño \textasciigrave{}n\textasciigrave{}. Se asume que \textasciigrave{}fun()\textasciigrave{} es una función de }
\CommentTok{\# una sola variable (no vectorial), \textasciigrave{}a \textless{} b\textasciigrave{} y \textasciigrave{}n\textasciigrave{} entero positivo.}
\CommentTok{\# ·········································································· }
...
en la que se emplean métodos numéricos\footnote{Como se comentó en la introducción, si el número de dimensiones es pequeño pueden ser más eficientes métodos numéricos. Podríamos considerar que la velocidad de convergencia de la aproximación por simulación es lenta pero no depende del número de dimensiones.}; ver Figura \ref{fig:int-num-1d} o \ref{fig:int-mc-gen}).
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{mc.integral0}\NormalTok{(fun, }\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{500}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Sin embargo, la función \texttt{mc.integral0()} no es adecuada para analizar la convergencia de la aproximación por simulación.
...
\begin{Highlighting}[]
\FunctionTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\FunctionTok{mc.integral}\NormalTok{(fun, }\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{5000}\NormalTok{, }\AttributeTok{ylim =} \FunctionTok{c}\NormalTok{(}\FloatTok{0.6}\NormalTok{, }\DecValTok{1}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\caption{Gráfico de convergencia de la aproximación mediante integración Monte Carlo clásica (con distribución uniforme).}\label{fig:mc-integral}
\end{figure}

Esta función devuelve además la aproximación por simulación y el error máximo (por defecto al 95\%).
Si sólo nos interesaran estos valores podemos establecer el argumento \texttt{plot\ =\ FALSE}.
Es importante tener en cuenta que la función \href{https://rubenfcasal.github.io/simres/reference/mc.integral.html}{\texttt{mc.integral()}} \textbf{solo es válida para dominio finito}.
\end{example}
...
Entonces, si \(x_1,x_2,\ldots ,x_n\) \emph{i.i.d.} \(X\), la aproximación por simulación será:
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{f }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(x) }\FunctionTok{ifelse}\NormalTok{((x }\SpecialCharTok{\textgreater{}=} \DecValTok{0}\NormalTok{) }\SpecialCharTok{\&}\NormalTok{ (x }\SpecialCharTok{\textless{}=} \DecValTok{1}\NormalTok{), }\DecValTok{4} \SpecialCharTok{*}\NormalTok{ x}\SpecialCharTok{\^{}}\DecValTok{3}\NormalTok{, }\DecValTok{0}\NormalTok{)}
\FunctionTok{curve}\NormalTok{(x}\SpecialCharTok{*}\FunctionTok{f}\NormalTok{(x), }\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{) }\CommentTok{\# curve(fun, 0, 1)}
\FunctionTok{curve}\NormalTok{(f, }\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, }\AttributeTok{lty =} \DecValTok{2}\NormalTok{, }\AttributeTok{col =} \StringTok{"blue"}\NormalTok{, }\AttributeTok{add =} \ConstantTok{TRUE}\NormalTok{)}
\FunctionTok{abline}\NormalTok{(}\AttributeTok{h =} \DecValTok{0}\NormalTok{, }\AttributeTok{lty =} \DecValTok{3}\NormalTok{)}
\FunctionTok{abline}\NormalTok{(}\AttributeTok{v =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{), }\AttributeTok{lty =} \DecValTok{3}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\caption{Representación del integrando de ejemplo (con dominio acotado) y de la densidad utilizada en su aproximación.}\label{fig:int-mc-gen}
\end{figure}

En la práctica, los pasos serían simular \texttt{x} con densidad \(f\) y aproximar la integral por \texttt{mean(h(x))}.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{rx }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(nsim) }\FunctionTok{runif}\NormalTok{(nsim)}\SpecialCharTok{\^{}}\NormalTok{(}\DecValTok{1}\SpecialCharTok{/}\DecValTok{4}\NormalTok{) }\CommentTok{\# Método de inversión}
\NormalTok{nsim }\OtherTok{\textless{}{-}} \DecValTok{5000}
\FunctionTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{x }\OtherTok{\textless{}{-}} \FunctionTok{rx}\NormalTok{(nsim)}
\CommentTok{\# h \textless{}{-} function(x) x}
\CommentTok{\# mean(h(x)) \# Aproximación por simulación }
\FunctionTok{mean}\NormalTok{(x)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Error máximo al 95\%}
\CommentTok{\# error \textless{}{-} 1.96*sd(h(x))/sqrt(nsim)}
\CommentTok{\# Gráfico de convergencia}
\CommentTok{\# plot(cumsum(h(x)/1:nsim, type = "l", lwd = 2, }
\CommentTok{\#      xlab = "Número de generaciones", ylab = "Media muestral")}
\end{Highlighting}
\end{Shaded}

Además de obtener la aproximación por simulación y el error máximo, por defecto al 95\%, podemos generar el gráfico de convergencia con \texttt{conv.plot(h(x))} (ver Figura \ref{fig:int-mc-gen-conv}):

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{res }\OtherTok{\textless{}{-}} \FunctionTok{conv.plot}\NormalTok{(x, }\AttributeTok{ylim =} \FunctionTok{c}\NormalTok{(}\FloatTok{0.6}\NormalTok{, }\DecValTok{1}\NormalTok{))}
\FunctionTok{abline}\NormalTok{(}\AttributeTok{h =} \DecValTok{4}\SpecialCharTok{/}\DecValTok{5}\NormalTok{, }\AttributeTok{lty =} \DecValTok{2}\NormalTok{, }\AttributeTok{col =} \StringTok{"blue"}\NormalTok{)}
\NormalTok{res}
\end{Highlighting}
\end{Shaded}
...
\caption{Gráfico de convergencia de la aproximación mediante integración Monte Carlo.}\label{fig:int-mc-gen-conv}
\end{figure}
...
\CommentTok{\# pnorm({-}4.5)  \# valor teórico P(X \textgreater{} 4.5) }
\FunctionTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{sum}\NormalTok{(x }\SpecialCharTok{\textgreater{}} \FloatTok{4.5}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Entonces, a partir de \(y_1,y_2,\ldots ,y_n\) \emph{i.i.d.} \(Y\sim g\), obtendríamos como aproximación:
...
\FunctionTok{curve}\NormalTok{(}\FunctionTok{dexp}\NormalTok{(x }\SpecialCharTok{{-}} \FloatTok{4.5}\NormalTok{) }\SpecialCharTok{*}\NormalTok{ escala, }\AttributeTok{add =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{lty =} \DecValTok{2}\NormalTok{, }\AttributeTok{col =} \StringTok{"blue"}\NormalTok{)  }
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{h }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(x) x }\SpecialCharTok{\textgreater{}} \FloatTok{4.5} 
\FunctionTok{mean}\NormalTok{(w}\SpecialCharTok{*}\FunctionTok{h}\NormalTok{(y))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# En este caso no sería necesario definir h}
\CommentTok{\# h(x) = 1 si x \textgreater{} 4.5 =\textgreater{} h(y) = 1}
\CommentTok{\# mean(w)}
...
Podemos representamos gráficamente la aproximación en función del número de simulaciones (\texttt{cumsum(w*h(y))/1:nsim}) y obtener medidas de la precisión, como el error estándar de la aproximación (\texttt{sqrt(var(w\ *\ h(y))/nsim)}), pero puede resultar más cómodo emplear \texttt{conv.plot(w*h(y))} (ver Figura \ref{fig:mc-imp-conv}):

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# plot(cumsum(w)/1:nsim, type = "l", ylab = "Aproximación", xlab = "Iteraciones")}
\CommentTok{\# sqrt(var(w)/nsim) \# sd(w*h(y))/sqrt(nsim) \# Error estándar}
\NormalTok{res }\OtherTok{\textless{}{-}} \FunctionTok{conv.plot}\NormalTok{(w)}
...
i.e.~\texttt{f(x)\ =\ dcauchy(x)} y \texttt{h(x)\ =\ (x\ \textgreater{}\ 2)\ *\ (x\ \textless{}\ 6)},
...
\NormalTok{w }\OtherTok{\textless{}{-}} \FunctionTok{dcauchy}\NormalTok{(y)}\SpecialCharTok{/}\FunctionTok{dnorm}\NormalTok{(y) }\CommentTok{\# w \textless{}{-} w/mean(w) si alguna es una cuasidensidad}
\end{Highlighting}
\end{Shaded}

La aproximación por simulación es \texttt{mean(w\ *\ h(y))}:
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{pcauchy}\NormalTok{(}\DecValTok{6}\NormalTok{) }\SpecialCharTok{{-}} \FunctionTok{pcauchy}\NormalTok{(}\DecValTok{2}\NormalTok{)  }\CommentTok{\# Valor teórico (normalmente desconocido)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# plot(cumsum(w * (y \textgreater{} 2) * (y \textless{} 6))/1:nsim, type = "l", }
\CommentTok{\#      ylab = "Aproximación", xlab = "Iteraciones")}
\FunctionTok{conv.plot}\NormalTok{(w }\SpecialCharTok{*}\NormalTok{ (y }\SpecialCharTok{\textgreater{}} \DecValTok{2}\NormalTok{) }\SpecialCharTok{*}\NormalTok{ (y }\SpecialCharTok{\textless{}} \DecValTok{6}\NormalTok{), }\AttributeTok{ylim =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\FloatTok{0.25}\NormalTok{))}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{boxplot}\NormalTok{(w}\SpecialCharTok{/}\FunctionTok{mean}\NormalTok{(w), }\AttributeTok{horizontal =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{log =} \StringTok{"x"}\NormalTok{)  }\CommentTok{\# boxplot(nsim * w/sum(w))}
\FunctionTok{abline}\NormalTok{(}\AttributeTok{v =} \DecValTok{1}\NormalTok{, }\AttributeTok{lty =} \DecValTok{3}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{mc-imp-otros}{%
\subsection{Otros métodos de muestreo por importancia}\label{mc-imp-otros}}
...
Aunque en general no es insesgado, puede ser más eficiente si \(w(Y)\) y \(w(Y)h(Y)\) están altamente correlacionadas \citep[e.g.][p.35]{liu2004}.
...
\begin{example}
\protect\hypertarget{exm:mc-imp3}{}\label{exm:mc-imp3}
...
\textbf{Nota}: En este caso \texttt{f(y)\ =\ dnorm(y)} y \texttt{g(y)\ =\ dcauchy(y)}, al revés que en el Ejemplo \ref{exm:mc-imp2} anterior.
...
\NormalTok{w }\OtherTok{\textless{}{-}} \FunctionTok{dnorm}\NormalTok{(y)}\SpecialCharTok{/}\FunctionTok{dcauchy}\NormalTok{(y) }
\CommentTok{\# w \textless{}{-} w/mean(w) si alguna es una cuasidensidad y coincidirían ambas aproximaciones}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# h(x) \textless{}{-} function(x) x}
\NormalTok{res }\OtherTok{\textless{}{-}} \FunctionTok{conv.plot}\NormalTok{(w}\SpecialCharTok{*}\NormalTok{y, }\AttributeTok{ylim =} \FunctionTok{c}\NormalTok{(}\SpecialCharTok{{-}}\FloatTok{0.05}\NormalTok{, }\FloatTok{0.05}\NormalTok{))}
...
Con los pesos normalizados la aproximación por simulación sería \texttt{sum(w\ *\ h(y))/sum(w)}:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{res }\OtherTok{\textless{}{-}} \FunctionTok{sum}\NormalTok{(w}\SpecialCharTok{*}\NormalTok{y)}\SpecialCharTok{/}\FunctionTok{sum}\NormalTok{(w)}
\NormalTok{res}
\end{Highlighting}
\end{Shaded}
...
Para el análisis de la convergencia se emplearía \texttt{cumsum(w\ *\ h(y))/cumsum(w)}:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{plot}\NormalTok{(}\FunctionTok{cumsum}\NormalTok{(w}\SpecialCharTok{*}\NormalTok{y)}\SpecialCharTok{/}\FunctionTok{cumsum}\NormalTok{(w), }\AttributeTok{type=}\StringTok{"l"}\NormalTok{, }\AttributeTok{ylab=}\StringTok{"Aproximación"}\NormalTok{, }
     \AttributeTok{xlab=}\StringTok{"Iteraciones"}\NormalTok{, }\AttributeTok{ylim =} \FunctionTok{c}\NormalTok{(}\SpecialCharTok{{-}}\FloatTok{0.05}\NormalTok{, }\FloatTok{0.05}\NormalTok{))}
\FunctionTok{abline}\NormalTok{(}\AttributeTok{h =}\NormalTok{ res)}
\CommentTok{\# conv.plot(w*y/mean(w), ylim = c({-}0.05, 0.05)) }
\CommentTok{\# Cuidado con la aproximación del error estándar}
\FunctionTok{abline}\NormalTok{(}\AttributeTok{h =} \DecValTok{0}\NormalTok{, }\AttributeTok{lty =} \DecValTok{2}\NormalTok{, }\AttributeTok{col =} \StringTok{"blue"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\textbf{Nota}: Si \(f\) o \(g\) fuesen cuasidensidades y se pidiese aproximar la integral, solo podríamos emplear esta última aproximación.
\end{example}

\hypertarget{mc-imp-resamp}{%
\subsection{Remuestreo (del muestreo) por importancia}\label{mc-imp-resamp}}
...
\FunctionTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\CommentTok{\# Sampling importance resampling}
\NormalTok{rx }\OtherTok{\textless{}{-}} \FunctionTok{sample}\NormalTok{(y, nsim2, }\AttributeTok{replace =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{prob =}\NormalTok{ w}\SpecialCharTok{/}\FunctionTok{sum}\NormalTok{(w))}
\CommentTok{\# Comparación distribuciones}
\FunctionTok{hist}\NormalTok{(rx, }\AttributeTok{freq =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{breaks =} \StringTok{"FD"}\NormalTok{, }\AttributeTok{main =} \StringTok{""}\NormalTok{, }\AttributeTok{ylim =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\FloatTok{0.5}\NormalTok{))}
\FunctionTok{lines}\NormalTok{(}\FunctionTok{density}\NormalTok{(rx))}
\FunctionTok{curve}\NormalTok{(dnorm, }\AttributeTok{col =} \StringTok{"blue"}\NormalTok{, }\AttributeTok{add =} \ConstantTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\textbf{NOTA}: Como se trata de una cuasi-densidad, \(f^{\ast}(x) = e^{-x}\cos^{2}(x)\), habrá que utilizar \(\tilde{\theta}_{g}\) mostrado \ref{mc-imp-otros}.
En el último apartado, para comprobar que los valores generados proceden de la distribución objetivo, si representamos la cuasidensidad junto con el histograma (en escala de densidades, \texttt{freq\ =\ FALSE}), hay que tener en cuenta que faltaría dividir la cuasidensidad por una constante normalizadora para poder compararlos directamente.
...
En este caso, como \(g\) es una densidad, podríamos estimar la constante normalizadora (en este caso \(c = 3/5\)) a partir de la media de los pesos del muestreo por importancia (\texttt{c.approx\ \textless{}-\ mean(w)}; ver Sección \ref{mc-imp-resamp}).
\end{exercise}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\textbf{\emph{LA MATERIA EVALUABLE EN EL CURSO 2025/2026 TERMINA AQUÍ}}
...
\protect\hypertarget{exm:mv-sann}{}\label{exm:mv-sann}

Repetimos el Ejemplo \ref{exm:mv-nlm} anterior empleando el método ``SANN'' de la función \texttt{optim()}:
...
La idea original (bootstrap natural, Efron) es que la variabilidad de \(\hat{\theta}_{b}^{\ast }\) en torno a \(\hat{\theta}\) aproxima la variabilidad de \(\hat{\theta}\) en torno a \(\theta\): \textbf{la distribución de} \(\hat{\theta}_{b}^{\ast }-\hat{\theta}\) (en el universo bootstrap) \textbf{aproxima la distribución de } \(\hat{\theta}-\theta\) (en la población).
...
\caption{Esquema de la idea del boostrap (de Efron).}\label{fig:unnamed-chunk-1}
\end{figure}

En general podríamos decir que \textbf{la muestra es a la población} \textbf{lo que la muestra bootstrap es a la muestra}.
...
\CommentTok{\# Caso unidimensional}
\NormalTok{fstatistic }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(data, i)\{}
\NormalTok{  remuestra }\OtherTok{\textless{}{-}}\NormalTok{ data[i]}
  \FunctionTok{mean}\NormalTok{(remuestra)}
\NormalTok{\}}

\NormalTok{stat.boot }\OtherTok{\textless{}{-}} \FunctionTok{boot.strap}\NormalTok{(muestra, B, fstatistic)}

\NormalTok{stat.dat }\OtherTok{\textless{}{-}} \FunctionTok{fstatistic}\NormalTok{(muestra, }\DecValTok{1}\SpecialCharTok{:}\FunctionTok{length}\NormalTok{(muestra))}
\NormalTok{res.boot }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(stat.dat, }\FunctionTok{mean}\NormalTok{(stat.boot)}\SpecialCharTok{{-}}\NormalTok{stat.dat, }\FunctionTok{sd}\NormalTok{(stat.boot))}
\FunctionTok{names}\NormalTok{(res.boot) }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"Estadístico"}\NormalTok{, }\StringTok{"Sesgo"}\NormalTok{, }\StringTok{"Error Std."}\NormalTok{)}
\NormalTok{res.boot}
\end{Highlighting}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Caso multidimensional}
\NormalTok{fstatisticm }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(data, i)\{}
\NormalTok{  remuestra }\OtherTok{\textless{}{-}}\NormalTok{ data[i, ]}
  \FunctionTok{cor}\NormalTok{(remuestra}\SpecialCharTok{$}\NormalTok{income, remuestra}\SpecialCharTok{$}\NormalTok{prestige)}
\NormalTok{\}}

\NormalTok{stat.boot }\OtherTok{\textless{}{-}} \FunctionTok{boot.strap}\NormalTok{(Prestige, B, fstatisticm)}
\end{Highlighting}
\end{Shaded}
...
\FunctionTok{hist}\NormalTok{(estadistico, }\AttributeTok{freq =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{main =} \StringTok{""}\NormalTok{, }\AttributeTok{breaks =} \StringTok{"FD"}\NormalTok{,  }
...
\FunctionTok{curve}\NormalTok{(n}\SpecialCharTok{/}\NormalTok{theta }\SpecialCharTok{*}\NormalTok{ ((x }\SpecialCharTok{+}\NormalTok{ theta)}\SpecialCharTok{/}\NormalTok{theta)}\SpecialCharTok{\^{}}\NormalTok{(n }\SpecialCharTok{{-}} \DecValTok{1}\NormalTok{), }\AttributeTok{col =} \StringTok{"blue"}\NormalTok{, }\AttributeTok{lty =} \DecValTok{2}\NormalTok{, }\AttributeTok{lwd =} \DecValTok{2}\NormalTok{, }
...
\protect\hypertarget{exm:media-par}{}\label{exm:media-par}
...
\CommentTok{\# Distribución bootstrap uniforme}
\CommentTok{\# plot(ecdf(muestra), xlim = c({-}.5, 3), ylab = "F(x)")}
...
\FunctionTok{legend}\NormalTok{(}\StringTok{"bottomright"}\NormalTok{, }\AttributeTok{legend =} \FunctionTok{c}\NormalTok{(}\StringTok{"Empírica"}\NormalTok{, }\StringTok{"Aprox. normal"}\NormalTok{, }\StringTok{"Aprox. exponencial"}\NormalTok{), }
...
\CommentTok{\# h \textless{}{-} npden$bw}

\CommentTok{\# plot(npden)}
\FunctionTok{hist}\NormalTok{(x, }\AttributeTok{freq =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{breaks =} \StringTok{"FD"}\NormalTok{, }\AttributeTok{main =} \StringTok{"Kernel density estimation"}\NormalTok{,}
     \AttributeTok{xlab =} \FunctionTok{paste}\NormalTok{(}\StringTok{"Bandwidth ="}\NormalTok{, }\FunctionTok{formatC}\NormalTok{(h)), }\AttributeTok{border =} \StringTok{"darkgray"}\NormalTok{, }
     \AttributeTok{xlim =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{80}\NormalTok{), }\AttributeTok{ylim =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\FloatTok{0.04}\NormalTok{))}
...
Podemos obtener este intervalo de confianza estableciendo \texttt{type\ =\ "norm"} (o \texttt{type\ =\ "all"}) en la llamada a la función \texttt{boot.ci()}.
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{boot.ci}\NormalTok{(res.boot, }\AttributeTok{type =} \StringTok{"norm"}\NormalTok{) }\CommentTok{\# index = 1}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{estim }\OtherTok{\textless{}{-}}\NormalTok{ op[index, }\DecValTok{1}\NormalTok{]}
\NormalTok{sesgo\_boot }\OtherTok{\textless{}{-}}\NormalTok{ op[index, }\DecValTok{2}\NormalTok{]}
\NormalTok{sd\_boot }\OtherTok{\textless{}{-}}\NormalTok{ op[index, }\DecValTok{3}\NormalTok{]}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{alfa }\OtherTok{\textless{}{-}} \FloatTok{0.05}
\NormalTok{z }\OtherTok{\textless{}{-}} \FunctionTok{qnorm}\NormalTok{(}\DecValTok{1} \SpecialCharTok{{-}}\NormalTok{ alfa}\SpecialCharTok{/}\DecValTok{2}\NormalTok{)}
\NormalTok{ic\_inf }\OtherTok{\textless{}{-}}\NormalTok{ estim }\SpecialCharTok{{-}}\NormalTok{ sesgo\_boot }\SpecialCharTok{{-}}\NormalTok{ z}\SpecialCharTok{*}\NormalTok{sd\_boot}
\NormalTok{ic\_sup }\OtherTok{\textless{}{-}}\NormalTok{ estim }\SpecialCharTok{{-}}\NormalTok{ sesgo\_boot }\SpecialCharTok{+}\NormalTok{ z}\SpecialCharTok{*}\NormalTok{sd\_boot}
...
Podemos obtener este intervalo de confianza estableciendo \texttt{type\ =\ "perc"} (o \texttt{type\ =\ "all"}) en la llamada a la función \texttt{boot.ci()}.
Continuando con el Ejemplo \ref{exm:media-dt-desconocida-boot}:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{boot.ci}\NormalTok{(res.boot, }\AttributeTok{type =} \StringTok{"perc"}\NormalTok{) }\CommentTok{\# index = 1}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{index }\OtherTok{\textless{}{-}} \DecValTok{1}
\NormalTok{stat\_boot }\OtherTok{\textless{}{-}}\NormalTok{ res.boot}\SpecialCharTok{$}\NormalTok{t[, index]}
\CommentTok{\# Utilizamos el mismo método que el paquete boot para aproximar los cuantiles:}
\NormalTok{IC\_boot }\OtherTok{\textless{}{-}}  \FunctionTok{quantile}\NormalTok{(stat\_boot, }\FunctionTok{c}\NormalTok{(alfa}\SpecialCharTok{/}\DecValTok{2}\NormalTok{, }\DecValTok{1} \SpecialCharTok{{-}}\NormalTok{ alfa}\SpecialCharTok{/}\DecValTok{2}\NormalTok{), }\AttributeTok{type =} \DecValTok{6}\NormalTok{)}
\CommentTok{\# (normalmente usaríamos el valor por defecto type = 7):}
\NormalTok{IC\_boot}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{hist}\NormalTok{(stat\_boot, }\AttributeTok{breaks =} \StringTok{"FD"}\NormalTok{, }\AttributeTok{freq =} \ConstantTok{FALSE}\NormalTok{)}
\FunctionTok{abline}\NormalTok{(}\AttributeTok{v =}\NormalTok{ IC\_boot)}
\end{Highlighting}
\end{Shaded}
...
Podemos obtener este intervalo de confianza estableciendo \texttt{type\ =\ "basic"} (o \texttt{type\ =\ "all"}) en la llamada a la función \texttt{boot.ci()}.

Siguiendo con el Ejemplo \ref{exm:media-dt-desconocida-boot}):

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{boot.ci}\NormalTok{(res.boot, }\AttributeTok{type =} \StringTok{"basic"}\NormalTok{) }\CommentTok{\# index = 1}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{index }\OtherTok{\textless{}{-}} \DecValTok{1}
\NormalTok{estim }\OtherTok{\textless{}{-}}\NormalTok{ res.boot}\SpecialCharTok{$}\NormalTok{t0[index]}
\CommentTok{\# Réplicas bootstrap del estadístico centrado:}
\NormalTok{stat\_boot }\OtherTok{\textless{}{-}}\NormalTok{ res.boot}\SpecialCharTok{$}\NormalTok{t[, index] }\SpecialCharTok{{-}}\NormalTok{ estim}
\CommentTok{\# Aproximación bootstrap de los ptos críticos}
\NormalTok{pto\_crit }\OtherTok{\textless{}{-}} \FunctionTok{quantile}\NormalTok{(stat\_boot, }\FunctionTok{c}\NormalTok{(alfa}\SpecialCharTok{/}\DecValTok{2}\NormalTok{, }\DecValTok{1} \SpecialCharTok{{-}}\NormalTok{ alfa}\SpecialCharTok{/}\DecValTok{2}\NormalTok{), }\AttributeTok{type =} \DecValTok{6}\NormalTok{)}
\CommentTok{\# Al igual que en el caso anterior, para obtener exactamente el mismo resultado,}
\CommentTok{\# utilizamos el mismo método que el paquete boot para aproximar los cuantiles}
\CommentTok{\# (normalmente usaríamos el valor por defecto type = 7)}

\CommentTok{\# Distribución bootstrap del estadístico centrado:}
\FunctionTok{hist}\NormalTok{(stat\_boot, }\AttributeTok{breaks =} \StringTok{"FD"}\NormalTok{, }\AttributeTok{freq =} \ConstantTok{FALSE}\NormalTok{)}
\FunctionTok{abline}\NormalTok{(}\AttributeTok{v =}\NormalTok{ pto\_crit)}

\CommentTok{\# Construcción del IC}
\NormalTok{ic\_inf\_boot }\OtherTok{\textless{}{-}}\NormalTok{ estim }\SpecialCharTok{{-}}\NormalTok{ pto\_crit[}\DecValTok{2}\NormalTok{]}
\NormalTok{ic\_sup\_boot }\OtherTok{\textless{}{-}}\NormalTok{ estim }\SpecialCharTok{{-}}\NormalTok{ pto\_crit[}\DecValTok{1}\NormalTok{]}
\NormalTok{IC\_boot }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(ic\_inf\_boot, ic\_sup\_boot)}
\FunctionTok{names}\NormalTok{(IC\_boot) }\OtherTok{\textless{}{-}} \FunctionTok{paste0}\NormalTok{(}\DecValTok{100}\SpecialCharTok{*}\FunctionTok{c}\NormalTok{(alfa}\SpecialCharTok{/}\DecValTok{2}\NormalTok{, }\DecValTok{1}\SpecialCharTok{{-}}\NormalTok{alfa}\SpecialCharTok{/}\DecValTok{2}\NormalTok{), }\StringTok{"\%"}\NormalTok{)}
\NormalTok{IC\_boot}
\end{Highlighting}
...
Si el segundo de los componentes del vector de estadísticos es una estimación de la varianza del primer componente (el estimador), obtendríamos este intervalo de confianza con las opciones por defecto (\texttt{type\ =\ "all"}) de la función \texttt{boot.ci()} o estableciendo \texttt{type\ =\ "stud"} (en cualquier caso siempre calcularía ``uno'' si hay al menos dos componentes en el vector de estadísticos, o generará un aviso si solo hay uno).

En caso de que el primer y segundo componente del vector de estadísticos no sean el estimador y su varianza estimada, respectivamente, habrá que establecer el argumento \texttt{index} igual a las posiciones correspondientes (vector de longitud 2).

Continuando con el Ejemplo \ref{exm:media-dt-desconocida-boot}):

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{boot.ci}\NormalTok{(res.boot, }\AttributeTok{type =} \StringTok{"stud"}\NormalTok{) }\CommentTok{\# index = c(1, 2)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{index }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{2}\NormalTok{)}
\NormalTok{estim }\OtherTok{\textless{}{-}}\NormalTok{ res.boot}\SpecialCharTok{$}\NormalTok{t0[index[}\DecValTok{1}\NormalTok{]]}
\NormalTok{var\_estim }\OtherTok{\textless{}{-}}\NormalTok{ res.boot}\SpecialCharTok{$}\NormalTok{t0[index[}\DecValTok{2}\NormalTok{]]}
\CommentTok{\# Réplicas bootstrap del estadístico studentizado:}
\NormalTok{stat\_boot }\OtherTok{\textless{}{-}}\NormalTok{ (res.boot}\SpecialCharTok{$}\NormalTok{t[, index[}\DecValTok{1}\NormalTok{]] }\SpecialCharTok{{-}}\NormalTok{ estim)}\SpecialCharTok{/}\FunctionTok{sqrt}\NormalTok{(res.boot}\SpecialCharTok{$}\NormalTok{t[, index[}\DecValTok{2}\NormalTok{]])}
\CommentTok{\# Aproximación bootstrap de los ptos críticos}
\NormalTok{pto\_crit }\OtherTok{\textless{}{-}} \FunctionTok{quantile}\NormalTok{(stat\_boot, }\FunctionTok{c}\NormalTok{(alfa}\SpecialCharTok{/}\DecValTok{2}\NormalTok{, }\DecValTok{1} \SpecialCharTok{{-}}\NormalTok{ alfa}\SpecialCharTok{/}\DecValTok{2}\NormalTok{), }\AttributeTok{type =} \DecValTok{6}\NormalTok{)}
\CommentTok{\# Al igual que en el caso anterior, para obtener exactamente el mismo resultado,}
\CommentTok{\# utilizamos el mismo método que el paquete boot para aproximar los cuantiles}
...
\CommentTok{\# Distribución bootstrap del estadístico studentizado:}
\FunctionTok{hist}\NormalTok{(stat\_boot, }\AttributeTok{breaks =} \StringTok{"FD"}\NormalTok{, }\AttributeTok{freq =} \ConstantTok{FALSE}\NormalTok{)}
\FunctionTok{abline}\NormalTok{(}\AttributeTok{v =}\NormalTok{ pto\_crit)}

\CommentTok{\# Construcción del IC}
\NormalTok{ic\_inf\_boot }\OtherTok{\textless{}{-}}\NormalTok{ estim }\SpecialCharTok{{-}}\NormalTok{ pto\_crit[}\DecValTok{2}\NormalTok{] }\SpecialCharTok{*} \FunctionTok{sqrt}\NormalTok{(var\_estim)}
\NormalTok{ic\_sup\_boot }\OtherTok{\textless{}{-}}\NormalTok{ estim }\SpecialCharTok{{-}}\NormalTok{ pto\_crit[}\DecValTok{1}\NormalTok{] }\SpecialCharTok{*} \FunctionTok{sqrt}\NormalTok{(var\_estim)}
\NormalTok{IC\_boot }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(ic\_inf\_boot, ic\_sup\_boot)}
\FunctionTok{names}\NormalTok{(IC\_boot) }\OtherTok{\textless{}{-}} \FunctionTok{paste0}\NormalTok{(}\DecValTok{100}\SpecialCharTok{*}\FunctionTok{c}\NormalTok{(alfa}\SpecialCharTok{/}\DecValTok{2}\NormalTok{, }\DecValTok{1}\SpecialCharTok{{-}}\NormalTok{alfa}\SpecialCharTok{/}\DecValTok{2}\NormalTok{), }\StringTok{"\%"}\NormalTok{)}
\NormalTok{IC\_boot}
\end{Highlighting}
...
\FunctionTok{hist}\NormalTok{(ht, }\AttributeTok{freq =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{breaks =} \StringTok{"FD"}\NormalTok{, }\AttributeTok{main =} \StringTok{""}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{hist}\NormalTok{(res.boot}\SpecialCharTok{$}\NormalTok{t, }\AttributeTok{freq =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{main =}\StringTok{""}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{hist}\NormalTok{(boot.mod}\SpecialCharTok{$}\NormalTok{t, }\AttributeTok{freq =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{breaks =} \StringTok{"FD"}\NormalTok{, }\AttributeTok{main =} \StringTok{""}\NormalTok{)}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.2208}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.2987}}@{}}
\toprule()
\begin{minipage}[b]{\linewidth}\raggedright
Clases
...
\FunctionTok{legend}\NormalTok{(}\StringTok{"bottomright"}\NormalTok{, }\AttributeTok{legend =} \FunctionTok{c}\NormalTok{(}\StringTok{"Datos independientes"}\NormalTok{, }\StringTok{"Datos dependientes"}\NormalTok{), }
       \AttributeTok{col =} \FunctionTok{c}\NormalTok{(}\StringTok{\textquotesingle{}blue\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}red\textquotesingle{}}\NormalTok{), }\AttributeTok{lty =} \DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{int-num-uni}{%
\section{Integración numérica unidimensional}\label{int-num-uni}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{fun }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(x) }\DecValTok{4} \SpecialCharTok{*}\NormalTok{ x}\SpecialCharTok{\^{}}\DecValTok{4}
...
\caption{Representación del integrando de ejemplo (con dominio acotado).}\label{fig:int-num-1d}
\end{figure}
