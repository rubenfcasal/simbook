...
La distribución doble exponencial^[Esta distribución también se puede generar fácilmente simulando una distribución exponencial y asignando un signo positivo o negativo con equiprobabilidad (ver Ejemplo \@ref(exm:dexp-mix)) y función `r cite_fgithub(rdexp, lnk = "ddexp", full = TRUE)` (fichero [*ddexp.R*](R/ddexp.R)).] (o distribución de Laplace) de
...
```{r cop-lambda, fig.cap="Representación de la cota óptima dependiedo del valor del parámetro y mínimo global (línea vertical)."}
...
a)  Generar una muestra i.i.d. $X_{i}\sim N(\theta_{0},1)$ de tamaño
    ...
    ```{r post-pri-plot, fig.cap="Comparación de la cuasi-densidad a posteriori (línea contínua) con la densidad a priori reescalada (línea discontinua)."}
    c.cuasi <- lik(mean(x))
    c.cuasi   
...
```{r squeeze, echo=FALSE, fig.cap = 'Ilustración del algoritmo de aceptación-rechazo con envolvente inferior (función "squeeze").'}
# ![](squeeze.png)
...
@gilks1992 propusieron una ligera modificación empleando tangentes para construir la cota superior, de esta forma se obtiene un método más eficiente pero requiere especificar la derivada de la densidad objetivo `r cite_fig(squeeze)`.
...
Ver por ejemplo el paquete `r cite_pkg(rust)`.
...
El correspondiente algoritmo está implementado en la función  `r cite_fgithub(rcauchy.rou)` del paquete `r cite_github()` (fichero [*ar.R*](R/ar.R)):
...
Este método está implementado en la función `r cite_fgithub(rdexp, lnk = "ddexp")` del paquete `r cite_github()` (fichero [*ddexp.R*](R/ddexp.R)).
