...
Como ya se ha visto anteriormente, en R podemos generar este gráfico con la función `r cite_fun(hist)` del paquete base. 
...
En R podemos obtener la función de distribución empírica con `r cite_fun(ecdf)` (el objeto que devuelve es una función).
...
Este método está implementado en la función `r cite_fun(chisq.test)` para el caso discreto (no corrige los grados de libertad).
Ejemplo:
...
Este método está implementado en la función `r cite_fun(ks.test)` del paquete base de R:
```{r eval=FALSE}
ks.test(x, y, ...)
```
...
de Lilliefors, implementado en la función `r cite_fun(lillie.test, pkg = "nortest")` del paquete `r cite_pkg(nortest)` 
...
```{r grafsec, fig.cap="Ejemplos de gráficos secuenciales.", fig.dim=c(10, 6), out.width = "90%"}
...
```{r fig.dim=c(10, 4), out.width = "100%"}
...
```{r dispret, fig.cap='Ejemplos de gráficos de dispensión retardados de dos secuencias de longitud 10000.', fig.dim=c(12, 6), out.width = "90%", message=FALSE, warning=FALSE, echo=FALSE}
library(randtoolbox)
n <- 10000
...
En R podemos calcular las autocorrelaciones, y generar el gráfico correspondiente, con la función `r cite_fun(acf)`.    
...
Este contraste está implementado en la función `r cite_fun(Box.test)`, estableciendo el argumento `lag` igual al número de saltos $m$ y `type = "Ljung"`.
