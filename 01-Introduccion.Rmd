...
```{r mc-plot-ealbum, fig.cap="(ref:mc-plot-ealbum)", fig.dim = c(10, 10), out.width="100%"}
...
Por ejemplo, en 1955 la Corporación RAND publicó el libro [*A Million Random Digits with 100,000 Normal Deviates*](https://www.rand.org/pubs/monograph_reports/MR1418.html) que contenía números aleatorios generados mediante una ruleta electrónica conectada a una computadora `r cite_fig(randbook)`.
...
```{r randbook, echo=FALSE, fig.cap="(ref:randbook)", out.width="30%"}
...
Por ejemplo, el dominio [RANDOM.ORG](http://www.random.org/integers) proporciona números "verdaderamente" aleatorios obtenidos a partir de ruido atmosférico (ver paquete `r cite_cran(random)` de R).
...
Por ejemplo, el paquete `r cite_cran(randtoolbox)` de R implementa métodos para la generación de secuencias cuasi-aleatorias `r cite_fig(randtoolbox)`.

```{r randtoolbox, fig.cap='Secuencias cuasi-aleatorias bidimensionales obtenidas con los métodos de Halton (izquierda), Sobol (centro) y Torus (derecha).', fig.dim = c(12, 5), out.width="100%", message=FALSE}
library(randtoolbox)
n <- 2000
...
-   `r cite_fun(set.seed)`: permite establecer la semilla (y el generador).

-   `r cite_fun(RNGkind)`: selecciona el generador.
...
-   `r cite_fun(sample)`: genera muestras aleatorias de variables discretas y permutaciones (se tratará en el Capítulo \@ref(discretas)).

-   `r cite_fun(simulate)`: genera realizaciones de la respuesta de un modelo ajustado.
...
Entre ellos podríamos destacar los paquetes `r cite_pkg_('distr', 'http://distr.r-forge.r-project.org')` (clases S4; con extensiones en otros paquetes) y `r cite_pkg_('distributions3', 'https://alexpghayes.github.io/distributions3/')` (clases S3)^[El paquete `r cite_pkg_('distr6', 'https://cran.r-project.org/package=distr6')`, que implementa clases R6, está actualmente archivado.].
...
se guardan y se procesan vectorialmente (normalmente empleando la función `r cite_fun(apply)`).
...
Para establecer estas opciones podemos emplear los argumentos `kind = NULL`, `normal.kind = NULL` y  `sample.kind = NULL` en las funciones `r cite_fun(RNGkind)` o `r cite_fun(set.seed)`.
...
-   `r cite_cran(setRNG)` contiene herramientas que facilitan operar con la semilla
    (e.g. dentro de funciones).

-   `r cite_cran(random)` permite la descarga de números “true random” desde
    [RANDOM.ORG](https://www.random.org).

-   `r cite_cran(randtoolbox)` implementa generadores más recientes (`rngWELL`) y
    generación de secuencias cuasi-aleatorias.

-   `r cite_cran(RDieHarder)` implementa diversos contrastes para el análisis de la
    calidad de un generador y varios generadores.

-   `r cite_cran(Runuran)` interfaz para la librería UNU.RAN para la generación
    (automática) de variables aleatorias no uniformes [ver @hormann2004].

-   `r cite_cran(rsprng)`, `r cite_cran(rstream)` y `r cite_cran(rlecuyer)` implementan
...
-   `r cite_fun(proc.time)`: permite obtener tiempo de computación real 
    y de CPU.
...
La función `r cite_fgithub(cpu.time)` del paquete `r cite_github()` implementa una aproximación similar:
...
Si se desea ir almacenando automáticamente los tiempos de computación, puede ser recomendable emplear las funciones `r cite_fun(tic, tictoc)` y `r cite_fun(toc, tictoc)` del paquete `r cite_pkg(tictoc)`:

- `tic("mensaje")`: inicia el temporizador y almacena el tiempo de inicio junto con el mensaje en una pila. 

- `r cite_fun(toc, tictoc)`: calcula el tiempo transcurrido desde la llamada correspondiente a `r cite_fun(tic, tictoc)`.
...
Para ello se pueden emplear las herramientas del paquete `r cite_pkg(microbenchmark)`.
No obstante, para los fines de este libro no será necesaria tanta precisión. 

Finalmente, si los tiempos de computación no fuesen asumibles, para identificar los cuellos de botella y mejorar el código para optimizar la velocidad, podríamos emplear la función `r cite_fun(Rprof)`. 
...
Después de ejecutar `Rprof(fichero)` y el código a evaluar, llamando a `Rprof(NULL)` se desactiva el muestreo y con `summaryRprof(fichero)` se muestran los resultados (para analizarlos puede resultar de utilidad el paquete `r cite_cran(proftools)`).
