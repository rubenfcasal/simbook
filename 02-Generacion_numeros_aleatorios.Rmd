...
Este método está implementado (no de la forma más eficiente[^02-gen-cong-1]) en la función `r cite_fgithub(rlcg, lnk ="set.rng")` del paquete `r cite_github()` (fichero [*rng.R*](R/rng.R)):
...
No obstante, en lugar de emplear esta función, puede resultar más cómodo utilizar `r cite_fgithub(set.rng)` para establecer la semilla (y opcionalmente los parámetros de este generador, que ya se selecciona por defecto con `type = "lcg"`) y posteriormente `r cite_fgithub(rng, lnk ="set.rng")` para generar múltiples secuencias.
...
Aunque también podemos emplear el siguiente código[^02-gen-cong-2] `r cite_fig(randu)`:
...
@marsaglia1968 demostró que las k-uplas de un generadores multiplicativo están contenidas en a lo sumo $\left(k!m\right)^{1/k}$ hiperplanos paralelos (`r trunc((factorial(3)*2^31)^(1/3))` como máximo con $k=3$ y $m=2^{31}$). 
...
con un período aproximado de $2^{129}$ y que puede ser empleado en R  estableciendo `kind` a `"Knuth-TAOCP"` en la llamada a `r cite_fun(set.seed)` o `r cite_fun(RNGkind)` (aunque sería preferible emplear `kind = "Knuth-TAOCP-2002"`; ver [Knuth Recent News 2002](https://www-cs-faculty.stanford.edu/~knuth/news02.html#rng)).
...
Uno de los contrastes más conocidos es el test chi-cuadrado de bondad de ajuste (`r cite_fun(chisq.test)` para el caso discreto; ver Sección \@ref(chi2test)). 
...
En este caso generamos `nsim = 1000` secuencias de longitud `n = 500` y empleamos la función `r cite_fgithub(freq.test)` con los valores por defecto (`nclass = 50`):
...
Este algoritmo está implementado en la función `r cite_fgithub(rvng, lnk ="set.rng", full =TRUE)` (ver también `r cite_fgithub(rng, lnk ="set.rng", full =TRUE)`; fichero [*rng.R*](R/rng.R)):
