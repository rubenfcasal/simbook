...
```{example, label="normind", name="simulación de normales independientes"}
```
...
```{conjecture, label="mnorm-indep", name="de simulación de normales independientes"}
...
```{example, label="ar-bidim", name="distribución bidimensional acotada"}
``` 
...
```{example, label="ar-esfera" , name="distribución uniforme en la esfera"}
``` 

Supongamos que el objetivo es simular puntos uniformemente distribuídos sobre la “esfera” unitaria $d$-dimensional `r citefig("simpiplot")`:
...
```{conjecture, label="mnorm-fact", name="de simulación de una normal multivariante" }
...
```{example, label="funcional", name="simulación de datos funcionales o temporales"}
```
...
```{conjecture, label="mult-distrcond", name="de simulación mediante distribuciones condicionadas" }
...
```{conjecture, label="norm-bidim-cond", name="de simulación de una normal bidimensional" }
...
```{conjecture, label="cond-incond", name="de simulación condicional" }
...
```{example, label = "funcionalcond", name="simulación condicional de datos funcionales o temporales"}
```
...
```{conjecture, label="copula-bidim", name="de simulación bidimensional mediante cópulas" }
...
```{exercise, label="clayton2d", name="Cópula bidimensional de Clayton"}
```
...
b)  Utilizando la rutina anterior generar una muestra de tamaño
    ...
    Representar la densidad conjunta (con `sm::sm.density()`) y las marginales`r latexfig("cclayton2b-dispersion", "cclayton3-dispersion")`:
    
    ```{r cclayton2-conjunta, message=FALSE, warning=FALSE, fig.cap="Densidad conjunta de los valores generados con distribución bidimensional de Clayton."}
    # Densidad conjunta
...
```{exercise, label="chicuadind", name="Distribución del estadístico chi-cuadrado de independencia"}
```
