...
Representamos la distribución de la muestra`r latexfig2(muestra-sim)`:
...
Alternativamente podríamos asumir un modelo paramétrico y estimar los parámetros a partir de la muestra (bootstrap paramétrico; Sección \@ref(modunif-boot-par)`r latexfig2(muestra-sim-aprox)`.
...
Como ejemplo consideramos la muestra de tiempos de vida de microorganismos `lifetimes` del paquete `simres``r latexfig2(microorganismos)`: 
...
de los resultados (correspondientes al estadístico determinado por el parámetro `index`, por defecto `= 1`): `r latexfig2(plot-res-boot)`
