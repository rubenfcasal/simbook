...
```{r, echo=FALSE, fig.cap = "Esquema de la idea del boostrap (de Efron)." }
...
Representamos la distribución de la muestra`r latexfig("muestra-sim")`:
```{r muestra-sim, fig.cap="Distribución de la muestra simulada (y distribución teórica en azul)."}
...
Alternativamente podríamos asumir un modelo paramétrico y estimar los parámetros a partir de la muestra (bootstrap paramétrico; Sección \@ref(modunif-boot-par)`r latexfig("muestra-sim-aprox")`.
```{r muestra-sim-aprox, fig.cap="Distribución teórica de la muestra simulada y distintas aproximaciones."}
...
```{r mean-boot-perc, fig.cap="Aproximación de la distribución de la media muestral centrada mediante bootstrap percentil (uniforme)."}
...
```{r mean-boot-basico, fig.cap="Aproximación de la distribución de la media muestral mediante bootstrap natural (uniforme)."}
...
Como ejemplo consideramos la muestra de tiempos de vida de microorganismos `lifetimes` del paquete `simres``r latexfig("microorganismos", output = is_latex())`: 
```{r microorganismos, fig.cap="Distribución del tiempo de vida de una muestra de microorganismos."}
library(simres)
muestra <- lifetimes
summary(muestra)
hist(muestra)
...
```{r mean-boot-stud, fig.cap="Aproximación de la distribución de la media muestral studentizada mediante bootstrap uniforme."}
...
de los resultados (correspondientes al estadístico determinado por el parámetro `index`, por defecto `= 1`): `r latexfig("plot-res-boot", output = is_latex())`

```{r plot-res-boot, fig.width=12, out.width="90%", fig.cap='Gráficos de diagnóstico de los resultados bootstrap para la media muestral de los tiempos de vida de microorganismos.'}
plot(res.boot)
```
...
```{r plot-boot-multi, fig.width=12, out.width="90%", fig.cap='Gráficos de diagnóstico para la distribución bootstrap del coeficiente de correlación.'}
library(boot)
...
```{r plot-boot-basico, fig.cap='Distribución bootstrap del estadístico percentil básico para el coeficiente de correlación.'}
