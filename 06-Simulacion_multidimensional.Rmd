...
Supongamos que el objetivo es simular puntos uniformemente distribuidos sobre la “esfera” (bola) unitaria $d$-dimensional `r cite_fig(simpiplot)`:
...
```{r pos-sp-simcond, fig.cap="Posiciones espaciales de las simulaciones condicionales (y las de los datos).", fig.dim=c(6, 7), out.width="70%"}
...
```{r krige-simcond, fig.cap="Simulaciones condicionales", out.width="90%"}
# Generar gráficos
...
(ref:co2-forecast) Concentración atmosférica de CO2 en el observatorio de Mauna Loa (desde el principio de 1990 hasta el final de 1997; ver `r cite_fun_("datasets::co2")`).

```{r co2-forecast, fig.cap="(ref:co2-forecast)"}
library(forecast)
...
Utilizando esta función generamos una muestra de tamaño 10000 y representamos gráficamente los valores obtenidos`r latexfig2(cclayton2-dispersion)`:
...
Podemos representar la densidad conjunta (con `sm::sm.density()`)`r latexfig2(cclayton2-conjunta)`:

```{r cclayton2-conjunta, fig.cap="Densidad conjunta de los valores generados con distribución bidimensional de Clayton", message=FALSE, warning=FALSE, fig.dim=c(10, 10)}
# if(!require(sm)) stop('Required package `sm` not installed.')
sm::sm.density(rcunif, xlab = "u", ylab = "v", zlab = "Density")    
```

y las distribuciones marginales`r latexfig2(cclayton2-marginales)`:

```{r cclayton2-marginales, fig.cap="Distribuciones marginales de los valores generados con distribución bidimensional de Clayton.", fig.dim=c(12, 6), out.width="90%"}
...
Empleando el paquete *copula* `r latexfig2(cclayton2b-dispersion, cclayton3-dispersion)`:
...
```{r cclayton3-dispersion, fig.cap="(ref:cclayton3-dispersion)", fig.dim=c(10, 10)}
...
```{r cclayton-exp-marginales, fig.cap="Distribuciones marginales exponenciales de los valores generados con dependencia definida por la cópula de Clayton.", fig.dim=c(12, 6), out.width="90%"}
