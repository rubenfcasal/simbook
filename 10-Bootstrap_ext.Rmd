...
$\mathcal{U}\left( 0,1\right)$ `r latexfig2(boot-uniforme-maximo)`:
...
En este caso parece razonable asumir una distribución exponencial (no lo es que el modelo admita tiempos de vida negativos, como ocurriría al asumir normalidad)`r latexfig2(boot-par-aprox)`:
...
del ejemplo original `r latexfig2(boot-parametrico-maximo)`:
...
Uno de los más empleados es `r cite_pkg(ks)` (Duong, 2019), que admite estimación incondicional y condicional multidimensional.
También se podrían emplear los paquetes `r cite_pkg(KernSmooth)` (Wand y Ripley, 2019), `r cite_pkg(sm)` (Bowman y Azzalini, 2019), `r cite_cran(np)` (Tristen y Jeffrey, 2019), `r cite_pkg(kedd)` (Guidoum, 2019), `r cite_cran(features)` (Duong y Matt, 2019) y `r cite_pkg(npsp)` (Fernández-Casal, 2019), entre otros.
...
Por ejemplo, en el caso de del conjunto de datos de precipitaciones, el siguiente código compara la estimación tipo núcleo de la distribución con la empírica`r latexfig2(pnp)`:

```{r pnp, fig.cap="Estimación empírica y tipo núcleo de la función de distribución de `precip`. "}
Fn <- ecdf(precip)
...
Por ejemplo, la función `density()` emplea por defecto un núcleo gaussiano, y como se muestra en la ayuda de esta función, podemos emplear un código como el siguiente para obtener `nsim` simulaciones`r cite_fig(density-sim)`:
